...
## ソースコードの説明
1.31 コマンド入力からデータ偽装に使う平均値を読み込む

1.32　コマンド入力からデータ偽装に使う分散を読み込む

1.33　コマンド入力から生成する偽装データ数を読み込む

1.35　コマンド行にあるスペースで区切られた文字列の数が4でない時は即座にプログラム終了

1.41～45　読み込んだデータを出力する

1.47　時計の時間をもとに乱数を生成

1.50　一様乱数関数を用いた標準正規乱数生成関数と入力した平均、分散の値をもとに条件にあった乱数を生成

1.51　乱数を出力する。

1.54　すべての処理が終わったらプログラム終了する
