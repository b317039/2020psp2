...
l.32: データファイル名を入力させ，fnameに格納

l.33: 入力させた文字列の最後にエンターキーが入っているので，fnameの文字列の最後をヌル文字'\0'に置換

l.34: fnameを確認

l.36: fnameで指定したファイルの先頭アドレスをファイルポインタ fp に読み込む

l.37: もしfpがNULLだったときは，ファイルを開けなかったので，即座にプログラム終了

l.42: ファイルを1行ずつbufに読み込む．もし読み込み成功する限り以下を繰り返す

l.43: bufからscanfで数値文字列を実数としてvalに読み込む

l.44: valに読み込まれたデータ、ループ回数を表すNからその地点での標本平均を求める

l.45: valの値を２乗して2乗の平均を求める準備をする

l.46: valの2乗の値からその地点での平均を求める

1.47： 1.45、1.46で求めた値から読み込まれた値の標本分散を求める

1.48：ループ回数を表すNを+1する

1.51：標本分散とデータ数+1を表すNから不偏分散を求める

l.53: ファイルを閉じる．閉じれないときは，即座にプログラム終了
...
[comment #20200428]
- (k01.c#L08) (k01.c#L12) プロトタイプ宣言には，externを付けますが，関数本体には付けません．
- ave,ave2は平均値絡みだと予想できますが，dis,dis2が分散だとは予想できません．名前を分散絡みだとわかるようにしてください．
- 出力に「不偏分散」とありますが，「不偏分散」は単なる計算方法です．ここでは「母集団分散（推定値）」としてください．
- これは，追加分ですが，Readme.mdに入出力をコピペする項を加えました．実際に動かした結果をコピペして提示してください．
